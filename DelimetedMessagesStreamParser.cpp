...
DelimetedMessagesStreamParser<MessageType>::parseDelimeted(const std::vector<char>& data, size_t size,
    ...
    return std::shared_ptr<MessageType>(new MessageType{msg});
}

template<typename MessageType>
size_t DelimetedMessagesStreamParser<MessageType>::parseSizeFromBuffer(std::vector<char> input) {
